 ...
 */
function copy_files(files_to_pack) {
  var files_to_pack = files_to_pack || pkg.files_to_pack;
  // console.log('files_to_pack : ', files_to_pack);
  var destination = process.env.TMPDIR + 'dist/';
  mkdir_sync(destination); // ensure that the /dist directory exists

  files_to_pack.forEach(function(file_path){
